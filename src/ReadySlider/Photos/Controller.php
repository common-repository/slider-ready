...
/**
 * Class ReadySlider_Photos_Controller
 *
 * @package ReadySlider\Photos
 */
class ReadySlider_Photos_Controller extends ReadySlider_Core_BaseController
{
    ...
    {
        return array(
            'resources' => 'ReadySlider_Galleries_Model_Resources',
            'photos'    => 'ReadySlider_Photos_Model_Photos',
            'folders'   => 'ReadySlider_Photos_Model_Folders',
            'position'  => 'ReadySlider_Photos_Model_Position',
        );
    }
    ...
    public function addAction(Rsc_Http_Request $request)
    {
        ...
        $photos = new ReadySlider_Photos_Model_Photos();
        ...
        if (!$photos->add($attachment->ID, $request->post->get('folder_id', 0))) {
            ...
            $response = array(
                ...
                'message' => sprintf(
                    $env->translate(
                        'Photo %s was successfully imported to the Gird Gallery'
                    ),
                    $attachment->post_title
                ),
            );
    ...
    {
        $env     = $this->getEnvironment();
        $folders = new ReadySlider_Photos_Model_Folders();
    ...
    public function isEmptyAction()
    {
        ...
        $isEmpty = true;
        $photos  = new ReadySlider_Photos_Model_Photos($debugEnabled);
