...
/**
 * Class ReadySlider_Photos_Model_Folders
 * Folders
 *
 * @package ReadySlider\Photos\Model
 * @author Artur Kovalevsky
 */
class ReadySlider_Photos_Model_Folders extends Rsc_Mvc_Model
{
    /**
     * @var string
     */
    ...
    {
        if (!class_exists('ReadySlider_Photos_Model_Photos', false)) {
            if ($this->debugEnabled) {
                wp_die (sprintf('The required class \'ReadySlider_Photos_Model_Photos\' is does not exists in method', __METHOD__));
            }

            return null;
        }

        $photos = new ReadySlider_Photos_Model_Photos($this->debugEnabled);
    ...
    public function getSliderImages($slider)
    {
        ...
        foreach ($slider->resources as $resource) {
            if ($resource->resource_type === 'folder') {
                ...
                if (class_exists('ReadySliderPro_Photos_Model_Videos')) {
                    $videos = new ReadySliderPro_Photos_Model_Videos();
