...
/**
 * Class ReadySlider_Core_BaseController
 * This class extends the default framework controller and adds some simple features to prevent duplication of the code
 *
 * @package ReadySlider\Core
 * @author Artur Kovalevsky
 */
class ReadySlider_Core_BaseController extends Rsc_Mvc_Controller
{
    ...
    {
        if (is_object($module)) {
            preg_match('/ReadySlider_(.*)_(.*)/', get_class($module), $matches);
     ...
     * @return Rsc_Mvc_Model|ReadySlider_Core_BaseModel
            ...
            throw new RuntimeException(
                sprintf(
                    $this->getEnvironment()->getLang()->translate(
                        'The model %s is does not registered with method ReadySlider_Core_BaseController::getModelAliases()'
                    ),
                    $modelName
                )
            );
